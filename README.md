...
- **不只是视频**:图片、Markdown / 文本笔记拖进来直接看,PDF 交给内置浏览器 ——
  「攻略」里图文其实比视频多,所以它也是块**置顶小抄**
- **最近观看**:控制条上的「最近」下拉栏,每条都带着上次的进度,一键接着看
- **本地视频记住播放进度**:看到一半关掉,下次打开同一个文件接着看(设置里可以关)
- **鼠标穿透**:按一下点击就穿过小窗落到游戏上(看教程视频时不影响操作),控制条上会挂个「穿透中」
- **窗口位置预设**:一键贴到屏幕某个角,游戏 UI 一般边角留空,正好放它
...
| 看图片 / 笔记 / PDF | 一样拖进来:图片和 `.md` / `.txt` 直接显示,PDF 交给内置浏览器(能滚动缩放) |
| 继续上次看的 | 控制条上的**「最近」下拉栏**,每条都带着上次看到哪儿 |
| 显示控制条 | 把鼠标移到**屏幕最顶端** |
| 移动窗口 | 在画面上按住拖动(网页模式下改成拖画面顶部那一条,见下文) |
| 把窗口挪到角上 | 设置 → 显示与声音 → 窗口位置:左上 / 右上 / 左下 / 右下 |
| 调整窗口大小 | 拖窗口的边或角 |
| 调音量 | 在画面上滚轮,或控制条上的音量条 |
| 打开设置 | 控制条最右边的齿轮图标 |
| 选集 / 切分P | 控制条上的**下拉栏**(标题右边):网页模式是 B 站的分P,本地模式是播放列表 / 图片列表 |
| 上一集 / 下一集 | 控制条上的 ◀◀ / ▶▶:网页模式下切的是 B 站分P |
| 切画质 / 开关弹幕 | 网页模式下控制条上的**「播放器」下拉栏**(按钮上显示当前画质) |
| 让点击穿过小窗 | <kbd>Ctrl</kbd>+<kbd>Alt</kbd>+<kbd>P</kbd>(开着时控制条上会挂一个「穿透中」标记) |
| 切到网页模式 | 设置里选「浏览器」(不给快捷键,免得在游戏里误触) |

设置窗口里可以改:**画面透明度、音量、播放倍速、全部全局热键、等比例拖动、循环播放、记住进度、窗口位置**。
...
| 倍速切换 | <kbd>Ctrl</kbd>+<kbd>Alt</kbd>+<kbd>S</kbd> |
| 音量 − / + | <kbd>Ctrl</kbd>+<kbd>Alt</kbd>+<kbd>C</kbd> / <kbd>Ctrl</kbd>+<kbd>Alt</kbd>+<kbd>V</kbd> |
| 鼠标穿透 开 / 关 | <kbd>Ctrl</kbd>+<kbd>Alt</kbd>+<kbd>P</kbd> |
...
- 控制条的**「播放器」下拉栏**把播放器自己的**画质**和**弹幕开关**搬了出来 ——
  切画质不用再去点小窗里那套面板。需要登录 / 大会员的档位点了不会生效,那是网站自己的判断。

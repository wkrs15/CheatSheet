...
| 打开设置 | 控制条最右边的齿轮图标 |
| 选集 / 切分P | 控制条上的**下拉栏**(标题右边):网页模式是 B 站的分P,本地模式是播放列表 |
...
- 打开 **B 站多P 视频**时,控制条上的下拉栏会自动列出**全部分P**(标题来自 B 站的公开接口,
  不用登录),点一下直接切集;在页面里自己切集,下拉栏也会跟着变。
  切集是**改网址**(`?p=N`)实现的,不依赖播放器面板的 DOM —— B 站改版也不容易失效。
